...
class DrishyaShravyaCsvImportView(APIView):
    """POST /api/v1/drishyashravyakosha/import-csv/ — admin-only.

    Lets an admin trigger the same import the `import_drishyashravya_csv`
    ...
    optionally `clear` ('true' to wipe existing entries first, matching the
    management command's --yes / confirmed path; omitted or anything else
    only imports rows whose subject isn't already in the database).
    """
