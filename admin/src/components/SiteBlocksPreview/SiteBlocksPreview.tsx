import React from 'react';
import type { SiteLandingBlock } from '@/services/api';
import { PROJECTS } from '@/config/projects';

// A non-interactive preview of the site-wide root home page's blocks —
// near-identical to LandingBlocksPreview.tsx, but the button target union
// differs (external/project instead of home/library/external), so it's
// kept as its own small component rather than sharing types.
const paragraphStyle: React.CSSProperties = {
  fontSize: 15, lineHeight: 1.8, color: 'var(--ps-text)', marginBottom: 18, whiteSpace: 'pre-line',
};

const buttonStyle: React.CSSProperties = {
  display: 'inline-block', background: 'var(--ps-grad)', color: '#fff', textDecoration: 'none',
  borderRadius: 999, padding: '11px 24px', fontWeight: 700, fontSize: 14, cursor: 'default',
};

interface Props {
  blocks: SiteLandingBlock[];
}

const SiteBlocksPreview: React.FC<Props> = ({ blocks }) => {
  if (blocks.length === 0) {
    return <p style={{ color: 'var(--ps-faint)', fontSize: 13 }}>Nothing to preview yet — add a paragraph or button.</p>;
  }

  return (
    <>
      {blocks.map((block, i) => {
        if (block.type === 'paragraph') {
          return (
            <p key={i} className="kn-sans" style={paragraphStyle}>
              {block.text || <span style={{ color: 'var(--ps-faint)' }}>(empty paragraph)</span>}
            </p>
          );
        }
        const targetHint = block.target_type === 'project'
          ? PROJECTS.find((p) => p.slug === block.project_slug)?.name || '(no project chosen)'
          : block.url || '(no URL set)';
        return (
          <div key={i} style={{ textAlign: 'center', margin: '20px 0' }}>
            <span className="kn-sans" style={buttonStyle}>
              {block.label || '(untitled button)'}
            </span>
            <div style={{ fontSize: 11, color: 'var(--ps-faint)', marginTop: 6 }}>&rarr; {targetHint}</div>
          </div>
        );
      })}
    </>
  );
};

export default SiteBlocksPreview;
